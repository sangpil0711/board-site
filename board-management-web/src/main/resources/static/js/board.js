 ...
 * BoardFactory의 query 메소드를 이용하여 게시글 목록 조회 및 검색, 페이지네이션 구현
 * 
 * 작성일 : 2023.09.01
 * 작성자 : 황상필
 */
app.controller("BoardCtrl", function($scope, BoardFactory, $window) {
	...
	let findPage = function() {
		BoardFactory.query({ pageNumber: $scope.currentPage, itemSize: $scope.itemsPerPage, searchType: $scope.searchType, keyword: $scope.keyword }, function(response) {
